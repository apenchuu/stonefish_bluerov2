...
void ROS2Interface::PublishEventBasedCamera(rclcpp::PublisherBase::SharedPtr pub, EventBasedCamera* ebc)
{
    ...
    for(size_t i=0; i<msg.events.size(); ++i)
    {
        ...
        {
            int32_t dt = std::abs(data[i*2+1]);
            builtin_interfaces::msg::Time ts = msg.header.stamp;
            int64_t nsec = static_cast<int64_t>(ts.nanosec) + static_cast<int64_t>(dt);
            ts.sec += static_cast<int32_t>(nsec / 1000000000LL);
            ts.nanosec = static_cast<uint32_t>(nsec % 1000000000LL);
            msg.events[i].ts = ts;
        }
